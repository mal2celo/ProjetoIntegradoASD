...
export class DatePickerMock extends DatePicker {
  ...
  show(options: DatePickerOptions): Promise<Date> {
    return new Promise((resolve, reject) => {
        this.data = new Date();
        this.presentAlertPrompt(resolve, options);
    });
  };

  async presentAlertPrompt(resolve, options: DatePickerOptions) {
    this.data = options.date;
      ...
      inputs: [
        {
          name: 'data',
          type: 'date',
          value: datePipe.transform(this.data,"yyyy-MM-dd")
        },
      ],
      buttons: [
        {
